...
def get_datas(runs, ref):
    ...
    df, fig = runs.plot_einsteiger(by=CODE, codes=ref.get_stations().CODE, ref_df=ref.get_stations())
    datas.append(SheetData(df, fig, "Einsteiger"))

    df, fig = runs.plot_einsteiger(by=CODE, codes=ref.get_stations().CODE, foreach=SUBPOPULATION)
